<?php class EZip extends CApplicationComponent {
    ...
    public function makeZip ($src, $dest)
    {
        ...
        if ($zip->open($dest, ZipArchive::CREATE) === true)
        {
            foreach ($src as $item)
                if (file_exists($item))
                    $this->addZipItem($zip, realpath(dirname($item)).$s, realpath($item).$s);
            $zip->close();
            return true;
        }
        return false;
    ...
    private function addZipItem ($zip, $racine, $dir)
    {
        if (is_dir($dir))
        {
            ...
            $lst = scandir($dir);
                ...
                array_shift($lst);
            foreach ($lst as $item)
                $this->addZipItem($zip, $racine, $dir.$item.(is_dir($dir.$item) ? DIRECTORY_SEPARATOR:''));
        }
        elseif (is_file($dir))
